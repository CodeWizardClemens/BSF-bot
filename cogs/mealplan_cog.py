...
class MealplanCog(commands.Cog):
    ...
    async def mealplan(self, ctx):
        ...
        if not self.kcals:
            ...
            tdee = calculate_tdee(calculate_bmr(self.bodyweight, self.height, self.age,
                                                self.gender), self.activity_level)
        ...
        self.macros["carbs"] = calculate_carbs(self.kcals, self.macros["fats"], self.macros["protein"])

        self.muscle_left_to_build = calculate_muscle_left_to_build(self.height, self.bodyfat, self.bodyweight)
        self.years_to_build_muscle = calculate_years_left_to_build_muscle(self.muscle_left_to_build)

        self.last_msg = (f"__**This is a summary of your general diet recommendations**__\n"
                       "```"
                       f"kcals: {self.kcals}\n"
    ...
    async def get_adjustment_string_based_on_goal(self):
        if self.goal == "Maintain":
            return ("You will need to track you weight to make sure you are maintaining it. "
    ...
    async def ask_kcals(self, ctx):
        await ctx.send("__**Do you know how many kcals you need?**__\n"
    ...
    async def ask_continue_meal_plan(self, ctx):
        await ctx.send("Do you want to continue with making a meal plan?\n"
    ...
    async def ask_bodyfat(self, ctx):
        await ctx.send("__**If you had to describe your bodyfat. Which best describes your body?**__\n"
    ...
    async def ask_goal(self, ctx):
        await ctx.send("__**Which of the follow numbers desribes your current goal the best?**__\n"
